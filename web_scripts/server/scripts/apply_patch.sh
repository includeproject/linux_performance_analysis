...
for file in $USER_PATCH_DIR; do
      ...
      then
	      ...
	      continue
              ...
              then
                     export APPLY_PATCH="sudo gunzip $file | sudo patch -p1";
                     sudo export APPLY_PATCH="sudo xz -dc $file | sudo patch -p1";
              elif [[ $file == *.bz2 ]]
              then
                     export APPLY_PATCH="sudo bunzip2 $file | sudo patch -p1";
                     sudo export APPLY_PATCH="sudo xz -dc $file | sudo patch -p1";
              fi
      fi
      echo "Trying to apply your patches" >> ./logs/vm/vm_status
      vagrant ssh -c "cd ~/linux && $APPLY_PATCH >> /vagrant/kernel/apply_patch_$local_patch_dir"
done;
